...
class DartCartPole2PoleEnv(dart_env.DartEnv, utils.EzPickle):
    def __init__(self):
        ...
        if not self.include_action_in_obs:
            self.net_modules.append([[1, 3], 0, None])
            self.net_modules.append([[0, 2], 0, [0]])
        else:
            self.net_modules.append([[1, 3, 5], 0, None])
            self.net_modules.append([[0, 2, 4], 0, [0]])

        self.net_modules.append([[], 1, [1]])
        self.net_modules.append([[], 1, [1, 0]])

        self.net_modules.append([[], None, [2, 3], None, False])
