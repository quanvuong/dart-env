...
class DartDogRobotEnv(dart_env.DartEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.assist_schedule = [[0.0, [84, 84]], [4.5, [63, 63.0]], [7.5, [47, 47]]]
    ...
    def _step(self, a):
        ...
        s = self.state_vector()
                    ...
                    abs(ang_cos_uwd) < 1.1) and (abs(ang_cos_fwd) < 1.2)
                    and np.abs(angle) < 1.1 and np.abs(self.robot_skeleton.q[5]) < 1.2 and np.abs(side_deviation) < 0.9 and not body_hit_ground)
