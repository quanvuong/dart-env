...
class DartClothUpperBodyDataDrivenClothPhaseInterpolateJacket2Env(DartClothUpperBodyDataDrivenClothBaseEnv, utils.EzPickle):
    def __init__(self):
        #feature flags
        rendering = True
        ...
        self.faceForwardReward          = False #shoulders facing -z #TODO: implement
        self.deformationPenalty         = True
        self.restPoseReward             = True
        self.rightTargetReward          = False
        self.leftTargetReward           = False
        self.clothPlacementReward       = False #reward closeness of efL to a vertex

        self.uprightRewardWeight        = 1
        self.stableHeadRewardWeight     = 1
        self.faceForwardReward          = 1
        ...
        #other flags
        self.sleeveFallTermination = False
        self.sleeveFallThreshold = 0.6
        ...
        self.resetDistributionPrefix = "saved_control_states_jacket/enter_seq_transition2"
    ...
    def checkTermination(self, tau, s, obs):
        ...
        if np.amax(np.absolute(s[:len(self.robot_skeleton.q)])) > 10:
            ...
            return True, -2000
        elif self.sleeveFallTermination: #terminate if sleeve slips off right arm
            sleeveProgress1 = pyutils.limbFeatureProgress(limb=pyutils.limbFromNodeSequence(self.robot_skeleton, nodes=self.limbNodesR[:3], offset=None), feature=self.sleeveRSeamFeature)
            sleeveProgress2 = pyutils.limbFeatureProgress(limb=pyutils.limbFromNodeSequence(self.robot_skeleton, nodes=self.limbNodesL[:3], offset=None), feature=self.sleeveLSeamFeature)
            print("sleeve Progress: [" + str(sleeveProgress1) + "," + str(sleeveProgress2) + "]")
            if sleeveProgress1 <= self.sleeveFallThreshold or sleeveProgress2 <= self.sleeveFallThreshold:
                return True, -2000
        return False, 0
    ...
    def additionalResets(self):
        ...
        self.restPose = np.zeros(len(self.robot_skeleton.q))
        self.restPose[8] = 0.21
        self.restPose[16] = 0.21
    ...
    def extraRenderFunction(self):
        ...
        if self.sleeveRSeamFeature is not None:
            self.sleeveRSeamFeature.drawProjectionPoly()
